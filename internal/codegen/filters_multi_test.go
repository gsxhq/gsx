...
func renderWithFilters(t *testing.T, myfilters string, views map[string]string, filterPkgs []string, invocation string) string {
	...
	gen, err := GeneratePackageWithFilters(viewsDir, filterPkgs, nil, nil, nil)
	if err != nil {
...
func TestMultiFilterUnknownErrors(t *testing.T) {
	...
	_, err = GeneratePackageWithFilters(viewsDir, []string{stdImportPath, "gsxmf/myfilters"}, nil, nil, nil)
	if err == nil {
