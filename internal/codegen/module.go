...
func (m *Module) Package(dir string) (*PackageResult, error) {
	...
	if len(a.typeErrs) == 0 {
		for _, f := range a.gsxFiles {
			generateFile(f, a.resolved, a.table, a.propFields, a.nodeProps, a.orderedProps, a.byo,
...
func (m *Module) Generate(dir string) (map[string][]byte, []diag.Diagnostic, error) {
	...
	if len(a.typeErrs) == 0 {
		for path, f := range a.gsxFiles {
			gen, ok := generateFile(f, a.resolved, a.table, a.propFields, a.nodeProps, a.orderedProps, a.byo,
