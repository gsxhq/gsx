...
- **Unused-import removal on format (a designed slice, not a tweak).** The
  type-checker already detects unused `.gsx` GoChunk imports (`"strings" imported
  and not used`), and main's `fix(codegen): map user-import type errors to .gsx
  source` already maps that diagnostic back to the `.gsx` import line (see
  `internal/codegen/import_diag_test.go` — `TestUnusedImportDiagnosticMapsToSource`,
  `…PerImportLine`). So **detection and source-mapping are done**; the remaining
  slice is the *edit*. Why it is still not a syntactic pass: a "scan for `pkg.`"
  approach is the exact heuristic to avoid — it mishandles aliased imports,
  packages whose name ≠ path tail, dedup, and blank/dot imports; the authoritative
  signal is the type-checker. The slice needs: reuse the mapped unused-import
  ranges; AST-level removal composed with the `gsxfmt` reprint; a guard that skips
  removal when the package has unrelated type errors (else a "used" import whose
  only use sits in errored code is falsely dropped); and an opt-in `gsx fmt` flag
  (default stays syntactic/fast — removal needs a module load, whereas plain
  formatting does not). Related cleanup: the overlay `*.x.go` diagnostics may
  still surface under a phantom `*.x.go` URI the user can't open — confirm
  post-merge and filter/remap if so.
