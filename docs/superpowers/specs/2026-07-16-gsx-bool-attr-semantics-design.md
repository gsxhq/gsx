...
### Where it lives (as shipped)

The **runtime** owns it: `gsx.IsBooleanAttr(name) bool` in the root package, over
three lists — `booleanAttrCore` (mechanically derivable), `presenceOnlyExtras`
(`hidden`, `download`), and a test-only guard (`contenteditable`, `draggable`,
`spellcheck` must never appear). It is a fixed HTML fact, not user-configurable,
so unlike the URL name sets it is a package var rather than caller-supplied.

Spread calls it directly; codegen imports the runtime and calls the *same*
function at generate time for the static `name={ boolExpr }` path. One source of
truth, consulted from both sides — no duplication, no threading. (Codegen
importing the runtime is a clean edge: the runtime is stdlib-only and never
imports codegen.)
...
**As shipped, `Spread` does NOT grow a parameter.** An earlier draft threaded a
`boolNames []string` beside the existing name sets. That was dropped: the
boolean-attribute list is a fixed HTML fact, not user-configurable like the URL
name sets, so threading a ~26-name literal into every Spread call site was pure
generated-code bloat. Instead the list lives in the runtime (`IsBooleanAttr`),
Spread calls it directly, and codegen imports the same function for the static
path — one source of truth, consulted from both sides, zero threading.
