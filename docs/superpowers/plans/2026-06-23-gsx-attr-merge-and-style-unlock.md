...
> **STATUS (2026-06-23):** Tasks **1, 2, 3, 3.5 LANDED** (last-wins `defaultClassMerge`; `gsx.ClassString`; CSS-context `={expr}` unlock for user-configured CSS attrs; `gsx.StyleValue` fixing the `style={ gsx.RawCSS(...) }` opt-out bug). Tasks **4 & 5 (the caller-wins fallthrough merge) are DEFERRED** to a separate, properly-designed slice. **Reason:** the architecture below ("build root attrs into an `Attrs` literal, `Merge(bag)`, `Spread`") is **unsafe as written** — `gw.Spread` only does plain HTML-attribute escaping (`AttrValue`), so folding URL-context (`gw.URL` scheme-sanitize) and JS-context (`gw.JSValAttr` double-escape) root attrs into it would drop their context escaping. Correct caller-wins needs a **context-aware runtime merge primitive** (resolve bag-over-root precedence AND escape each attribute by its context) — a design problem, not a mechanical rewrite. T1–T3.5 stand on their own as a "class/style merge building blocks + CSS-attr correctness" slice. NOTE: built-in `style={expr}` was already sanitized (parses to a composed `ClassAttr` → `emitStyleAttr`/`FilterCSS`), so it was never fail-closed — only user-configured CSS-context attrs were, now unlocked.
