...
- **Deferred:** references and definitions sourced FROM external/non-project
  packages — their files are not indexed, so a cursor in one is not answered and
  their use sites are absent from a reference list. Go to definition from a
  `.gsx` cursor INTO an external package (a `|> upper` filter in `gsx/std`, a
  dependency's component) does resolve.
