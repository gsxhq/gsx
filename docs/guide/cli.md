...
Deleting a `.gsx` removes its generated `.x.go` on the next `gsx generate` (or
the next `gsx dev` regen) — including a poison file left behind by a previous
failure. Only files gsx itself generated are ever removed (recognized by the
`// Code generated by gsx; DO NOT EDIT.` header on line 1); a hand-written file
that happens to be named `*.x.go` is never touched.
