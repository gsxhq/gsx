...
`srcset` (and `imagesrcset`, its `<link rel=preload>` counterpart) is not a single URL. gsx parses it as the WHATWG `srcset` grammar — a comma-separated list of image candidates (`url descriptor`) — and sanitizes each candidate's URL individually as an [image resource](./escaping.md#resource-vs-navigational-url-sinks): a disallowed scheme in any one candidate collapses just that candidate to `about:invalid#gsx`, leaving the rest of the list untouched. `data:image/*` candidates and fractional density descriptors (`1.5x`) are preserved. This holds identically whether `srcset` is written as a static attribute or arrives through a `{ bag… }` spread.
