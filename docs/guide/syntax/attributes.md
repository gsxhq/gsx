...
A spread `{ x… }` onto **any** element sanitizes URL-typed attributes exactly like a static attribute of the same name would, regardless of the bag's provenance — the implicit `attrs` bag, a byo component's declared `Attrs` field, a generated component's own named `gsx.Attrs` param, a **local variable** (a `gsx.Attrs` declared and assigned inside the component body), a **function call**'s result (`{ Source()... }`), a **byo struct's second `gsx.Attrs` field**, a derived bag (`{ b.Without("id")... }`), or one nested inside a conditional-attr group (`{ if cond { { x... } } }`). Generated code emits a single `gsx.SpreadForwarding(...)` call that, in one ordered walk over the bag, matches every URL-classified key (`href`, `src`, `action`, and the rest of the [URL-attribute table](#contextual-escaping), plus any project `[[urlAttrs]]` rule) case-insensitively and writes it through the matching tag-aware sink (navigational or image-resource) in place — at the bag's own authored position, not hoisted ahead of the rest of the bag. Wrap an already-validated value in `gsx.RawURL(...)` to skip the scheme check for that one attribute.

This sanitization is universal, not a special case for a particular bag spelling: every element spread routes through the same `SpreadForwarding` call, so there is no unsanitizing spread primitive and no bag provenance that stays inline/unsanitized. See [Composition — Precedence](./composition.md#precedence) and the security note on `gsx.Attrs`'s doc comment for the full contract.
