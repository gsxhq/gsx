...
::: v-pre
Every attribute on an attrs-only tag — bare, spread, conditional, or the `attrs={{ … }}` literal — lands in that one `gsx.Attrs` bag, exactly like the synthesized fallthrough bag above, and it is **not** a separate security regime: sanitization is decided by whatever actually spreads the bag onto an element, not by how the bag got there. In the icon-wrapper pattern below, that's `renderIcon`'s own declared `Attrs` field — spread through `p.Attrs`, even composed inside `.Merge(...)` as it is there — which is a forwarding position and gets the same leaf URL extraction as any other declared bag (see [Attributes — Spread](./attributes.md#spread)). The same recognition applies when a component value's own body embeds an element literal directly (no delegating `component` in between) and spreads its parameter under the literal identifier `attrs`, matching gsx's fallthrough-bag recognition. Spread a differently-named parameter directly, with no delegating component in between, and it stays an inline, position-blind bag instead — the same [local-bag exception](./composition.md#precedence) a body-local `gsx.Attrs` variable gets: no defaults/forced precedence, no class merge, no URL extraction. Wrap an already-validated value in `gsx.RawURL(...)` to opt out of the scheme check wherever sanitization does apply.
:::
