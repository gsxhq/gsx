...
| **Attribute-local CSS** | `` attr=css`...` `` or `` attr={css`...`} ``; `@{ expr }` holes inside the literal | Preserve the surrounding CSS and filter each hole as a CSS value before attribute-escaping the result | `gsx.RawCSS(s)` in a hole |
