...
func TestDefinitionSignatureParamTypeCrossPkg(t *testing.T) {
	...
	// Case 2: `store` qualifier → the import in the .gsx file.
	loc = sigTypeDefAt(t, dir, "blog/comp.gsx", src, sigLine, storeCol)
	if loc == nil {
		t.Fatalf("gd on `store` qualifier returned null")
	}
	if !strings.HasSuffix(loc.URI, "comp.gsx") {
		t.Fatalf("`store` resolved to %q, want comp.gsx (the import)", loc.URI)
	}
}
